...
# Inner comprehension crates a single row
EXAMPLE_GRID = [[row + col for col in range(GRID_WIDTH)]
			   for row in range(GRID_HEIGHT)]

def traverse_grid(start_cell, direction, num_steps):
    """
    Function that iterate through the cells in a grid
    in a linear direction

    Both start_cell is a tuple(row, col) denoting hte 
    starting cell

    direction is a tuple that contains difference between
    consecutive cells in hte traversal
    """
